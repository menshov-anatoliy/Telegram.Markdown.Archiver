...
public class TelegramArchiverHostedService : BackgroundService
{
    ...
    private async Task ProcessMessageAsync(Message message)
    {
        try
        {
            // Проверить, что сообщение из нужного чата (Saved Messages)
            if (message.Chat.Id != _telegramConfiguration.UserId)
            {
                _logger.LogDebug("Пропуск сообщения из чата {ChatId} (ожидается {UserId})", 
                    message.Chat.Id, _telegramConfiguration.UserId);
                return;
            }
            ...
            if (message.Type == MessageType.Voice && message.Voice != null)
            {
                ...
                {
                    await _telegramService.SendTextMessageAsync(_telegramConfiguration.UserId, transcription);
                }
            }
