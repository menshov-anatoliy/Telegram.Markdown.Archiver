	...
	: ITelegramService
{
	...
	public async Task<Update[]> GetUpdatesAsync(int offset = 0)
	{
		try
		{
			logger.LogDebug("Получение обновлений с offset: {Offset}", offset);
			...
			logger.LogInformation("Получено {Count} обновлений", updates.Length);
			return updates;
		}
		catch (Exception ex)
		{
	...
	public async Task<byte[]?> DownloadFileAsync(string fileId)
	{
		try
		{
			...
			{
				logger.LogWarning("Не удалось получить информацию о файле {FileId}", fileId);
				return null;
			}
			...
			logger.LogInformation("Файл {FileId} загружен, размер: {Size} байт", fileId, stream.Length);
	...
	public async Task SendTextMessageAsync(long chatId, string text)
	{
		try
		{
			...
			logger.LogInformation("Сообщение отправлено в чат {ChatId}: {Text}", chatId, text.Substring(0, Math.Min(text.Length, 50)));
		}
		catch (Exception ex)
		{
			logger.LogError(ex, "Ошибка при отправке сообщения в чат {ChatId}", chatId);
			// НЕ логируем эту ошибку в Telegram, чтобы избежать рекурсии
		}
	}
	...
	public async Task<object?> GetFileAsync(string fileId)
	{
		...
		{
			var file = await _botClient.GetFile(fileId);
			logger.LogDebug("Получена информация о файле {FileId}: {FilePath}", fileId, file.FilePath);
			return file;
		}
		catch (Exception ex)
		{
